...
function testSimpleSelectQueryWithSimpleVariable() returns int[]{
    //
  ...
  //OddNumberGenerator oddGen = new ();
    ...
    //int[] outputList =
    //        from int number in oddNumberStream
    //        select number;

    //stream<Person> streamedPersons = personList.toStream();
    //Person[] outputPersonList2 = [];

var intStream = intArr.toStream();
int[] outputList = [];
        error? err = intStream.forEach(function (int num) {
            outputList[outputList.length()] = num;
        });


var intStream2 = intArr.toStream();

int[] outputList2 = from int i in intStream2
select 2;

    io:println(outputList2);
    //if(err is error) {
    //    outputList = err;
    //}

  //  if(err is ()) {
  //outputPersonList2[1] = err;
  //     }
